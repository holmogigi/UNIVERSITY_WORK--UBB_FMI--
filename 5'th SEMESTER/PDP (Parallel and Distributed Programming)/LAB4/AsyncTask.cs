...
namespace LAB4
{
    public class AsyncTasks
    {
        ...
        private static async Task Connect(Obj state)
        {
        ...
        private static async Task Send(Obj state, string data)
        {
        ...
        private static async Task Receive(Obj state)
        {
            // begin receiving the data from the server
            state.socket.BeginReceive(state.receiveBuffer, 0, Obj.BUFFER_SIZE, 0, ReceiveCallback, state);
        ...
        private static void ReceiveCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                {
                    clientSocket.BeginReceive(state.receiveBuffer, 0, Obj.BUFFER_SIZE, 0, ReceiveCallback, state);
                }
                else
                {
